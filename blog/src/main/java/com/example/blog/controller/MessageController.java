package com.example.blog.controller;

import com.example.blog.entity.Message;
import com.example.blog.entity.User;
import com.example.blog.mapper.MessageMapper;
import com.example.blog.mapper.UserMapper;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.CrossOrigin;
import org.springframework.web.bind.annotation.RequestBody;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RestController;

import java.util.HashMap;
import java.util.List;
import java.util.Map;

@CrossOrigin
@RestController
@RequestMapping("/message")
public class MessageController {

    @Autowired
    private MessageMapper messageDao;

    @Autowired
    private UserMapper userDao;

    /**
     * 获取列表
     *
     * @param
     * @return
     */
    @RequestMapping("/list")
    public Map list(@RequestBody Message message) {
        Map result = new HashMap();
        List<Message> list = messageDao.getList(message);
        for (int i = 0; i < list.size(); i++) {
            User user = userDao.getById(list.get(i).getUid());
            list.get(i).setName(user!=null?user.getName():"游客");
        }
        result.put("list",list);
        result.put("count",list.size());
        return result;
    }


    /**
     * 新增
     *
     * @param
     * @return
     */
    @RequestMapping("/add")
    public Map add(@RequestBody Message message) {
        Map result = new HashMap();
        messageDao.add(message);
        result.put("msg","新增成功");
        result.put("code",200);
        return result;
    }


    /**
     * 修改
     *
     * @param
     * @return
     */
    @RequestMapping("/edit")
    public Map edit(@RequestBody Message message) {
        Map result = new HashMap();
        messageDao.update(message);
        result.put("msg","修改成功");
        result.put("code",200);
        return result;
    }


    /**
     * 删除
     *
     * @param
     * @return
     */
    @RequestMapping("/del")
    public Map del(@RequestBody Message message) {
        Map result = new HashMap();
        messageDao.del(message);
        result.put("msg","删除成功");
        result.put("code",200);
        return result;
    }




}